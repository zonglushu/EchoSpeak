/**
 * P0-1: Daily Check-in System - Check-in Calendar V2
 * 参考 GitHub 贡献图 + Duolingo 打卡日历设计
 * 显示更长时间范围（3个月），使用热力图（Heatmap）展示
 */
...
export const CheckinCalendarV2: React.FC<CheckinCalendarV2Props> = ({
  ...
  const [viewMonths, setViewMonths] = useState<number>(3); // 显示月数：3 或 12

  useEffect(() => {
    loadCalendarData();
  }, [userId, useDemoData, viewMonths]);
  ...
  const loadCalendarData = async () => {
    ...
    try {
      const checkins = await getCheckinCalendar(userId, viewMonths);
  ...
  const generateDemoData = (): DayData[] => {
    ...
    for (let i = daysToShow - 1; i >= 0; i--) {
      ...
      if (isWeekend) {
        ...
        // 工作日：85% 概率练习，时长中等
        if (random > 0.15) {
          duration = Math.floor(Math.random() * 2400) + 1200; // 20-60分钟
          hasData = true;
        }
      }
  ...
  // 获取热力图颜色 - 5 个等级
  const getHeatmapColor = (day: DayData): string => {
    if (!day.hasData) {
...
export default CheckinCalendarV2;
